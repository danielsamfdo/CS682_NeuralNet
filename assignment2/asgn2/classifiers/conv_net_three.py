...
class ConvNetThree(object):
  ...
  def loss(self, X, y=None):
    ...
    # variable.                                                                #
    ############################################################################
    first_layer_output, first_layer_cache = conv_bnorm_relu_pool_forward(X,W1,b1,conv_param,pool_param,self.bn_param,"_conv_layer_1")
    # print first_layer_output.shape
    first_layer_1_output, first_layer_1_cache = conv_bnorm_relu_pool_forward(X,W2,b2,conv_param,pool_param,self.bn_param,"_conv_layer_2")
    # print first_layer_1_output.shape
    first_layer_2_output, first_layer_2_cache = conv_bnorm_relu_pool_forward(X,W3,b3,conv_param,pool_param,self.bn_param,"_conv_layer_3")
    print first_layer_2_output.shape
    ...
    second_layer_output, second_layer_cache = affine_batch_norm_relu_forward(first_layer_2_output,W4,b4,self.bn_param,"_affine_layer_1")
    ...
    reg = self.reg
    loss += (0.5 * reg * np.sum(W1*W1)) + (0.5 * reg * np.sum(W2*W2)) + (0.5 * reg * np.sum(W3*W3))+ (0.5 * reg * np.sum(W4*W4))+ (0.5 * reg * np.sum(W5*W5))

    dthird_layer, grads['W5'], grads['b5']  = affine_backward(dout, third_layer_cache)
    dsecond_layer, grads['W2'], grads['b4']  = affine_batch_norm_relu_backward(dthird_layer, second_layer_cache)
    dfirst_layer_2, grads['W3'], grads['b3'] = conv_bnorm_relu_pool_backward(dsecond_layer, first_layer_2_cache)
    dfirst_layer_1, grads['W2'], grads['b2'] = conv_bnorm_relu_pool_backward(dfirst_layer_2, first_layer_1_cache)
    dfirst_layer, grads['W1'], grads['b1'] = conv_bnorm_relu_pool_backward(dfirst_layer_1, first_layer_cache)
