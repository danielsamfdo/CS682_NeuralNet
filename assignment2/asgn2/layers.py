...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    #############################################################################
    X = x
    sqrt_variance = np.sqrt(running_var+eps)
    out = (X - running_mean)
    out = out/sqrt_variance
    X_cap = np.copy(out)
    out = (out*gamma) + beta
    cache = (X, X_cap, gamma, beta, running_var, running_mean,eps)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
  else:
...
def batchnorm_backward(dout, cache):
  ...
  dxcap = np.copy(dout)*gamma
  dvariance = np.sum(dxcap * (X-X_bar) * (-0.5 * ((variance+eps)**(-1.5))), axis=0)
  dmean = np.sum(-1*dxcap, axis=0)/np.sqrt(variance+eps) + dvariance  * np.sum(((X-X_bar) * -2),axis=0)/m
...
def conv_forward_naive(x, w, b, conv_param):
  ...
  for train_i in range(N):
    ...
    padded_x_H, padded_x_W = input_i[0].shape
    for conv_layer in range(F):
      conv = w[conv_layer]
      c, conv_h, conv_w = conv.shape
      i=0
      row = 0
      while(row+conv_h<=padded_x_H):
        column = 0
        j=0
        while(column+conv_w<=padded_x_W):
          out[train_i,conv_layer,i,j] += np.sum(input_i[:,row:row+conv_h,column:column+conv_w] * conv[:]) 
          out[train_i,conv_layer,i,j] += b[conv_layer]
  ...
  for train_i in range(x.shape[0]):
    ...
    for filt in range(f):
      for channel in range(channels):
        i=0
        row = 0
        conv_h, conv_w = w[filt,channel].shape
        while(row+conv_h<=padded_x_H):
          column = 0
          j=0
          while(column+conv_w<=padded_x_W):
